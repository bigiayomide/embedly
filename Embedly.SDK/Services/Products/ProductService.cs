...
internal sealed class ProductService : BaseService, IProductService
{
    ...
    /// <inheritdoc />
    public async Task<ApiResponse<PaginatedResponse<Product>>> GetProductsAsync(GetProductsRequest request,
        CancellationToken cancellationToken = default)
    {
        Guard.ThrowIfNull(request, nameof(request));

        var url = BuildUrl(ServiceUrls.Base, "api/v1/products/get");
        return await HttpClient.GetAsync<PaginatedResponse<Product>>(url, request.ToQueryParameters(),
            cancellationToken);
    }

    /// <inheritdoc />
    public async Task<ApiResponse<Product>> UpdateProductAsync(string productId, CreateProductRequest request,
        CancellationToken cancellationToken = default)
    {
    ...
    /// <inheritdoc />
    public async Task<ApiResponse<Product>> ActivateProductAsync(string productId,
        CancellationToken cancellationToken = default)
    {
        Guard.ThrowIfNullOrWhiteSpace(productId, nameof(productId));
    ...
    /// <inheritdoc />
    public async Task<ApiResponse<Product>> DeactivateProductAsync(string productId,
        CancellationToken cancellationToken = default)
    {
        Guard.ThrowIfNullOrWhiteSpace(productId, nameof(productId));
    ...
    /// <inheritdoc />
    public async Task<ApiResponse<object>> DeleteProductAsync(string productId,
        CancellationToken cancellationToken = default)
    {
        Guard.ThrowIfNullOrWhiteSpace(productId, nameof(productId));
    ...
    /// <inheritdoc />
    public async Task<ApiResponse<Models.Responses.Products.ProductLimits>> GetDefaultLimitsAsync(
        CancellationToken cancellationToken = default)
    {
    ...
    /// <inheritdoc />
    public async Task<ApiResponse<Models.Responses.Products.ProductLimits>> GetProductLimitsAsync(Guid productId,
    ...
    /// <inheritdoc />
    public async Task<ApiResponse<Models.Responses.Products.ProductLimits>> GetProductLimitsByCustomerAsync(
        Guid productId, Guid currencyId, string customerId, CancellationToken cancellationToken = default)
    {
        Guard.ThrowIfNullOrWhiteSpace(customerId, nameof(customerId));

        var url = BuildUrl(ServiceUrls.Base,
            $"api/v1/limits/product/{productId}/currency/{currencyId}/customer/{customerId}");
        return await HttpClient.GetAsync<Models.Responses.Products.ProductLimits>(url, cancellationToken);
    }
}